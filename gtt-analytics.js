...
class ProductHandler {
    ...
    setBasicProductInfo(config, data) {
        if (data.item) config.productInfo.product_name = this.decode(data.item).trim()
        if (data.itemModel) config.productInfo.product_model = this.decode(data.itemModel)
        if (data.itemYear > 0) config.productInfo.product_year = data.itemYear
        if (data.itemIndustry)
            config.productInfo.product_industry = this.decode(data.itemIndustry).trim()
    ...
    setProductCategories(config, data) {
        if (data.itemTypeId) {
            config.productInfo.product_category = this.decode(data.itemType)
        ...
        if (data.itemSubtypeId) {
            ...
            config.productInfo.product_subcategory = this.decode(data.itemSubtype)
        }

        if (data.itemMakeId) {
            config.productInfo.product_make_id = data.itemMakeId
            config.productInfo.product_make = this.decode(data.itemMake)
        }
    }
        ...
        $('span.datasource.hidden').each(function () {
            try {
                ...
                if (self.isValidProductData(data, productItemFields)) {
                    // Deduplicate: the DOM may render the same span in multiple layout
                    // containers (e.g. grid view and list view). Only add each product once.
                    // Key = productId + isUnitInventory so that the same productId appearing
                    // as both Inventory and Showcase types is treated as distinct.
                    // e.g. "1234_true" (Inventory) vs "1234_false" (Showcase)
                    const uniqueKey = `${String(data.productId || '')}_${String(data.isUnitInventory || '')}`
    ...
    setBasicProductInfoFromItem(item, product) {
        if (item.item) product.product_name = this.decode(item.item)
        if (item.itemModel || item.model) {
            product.product_model = this.decode(item.itemModel || item.model)
        }
        ...
        if (item.itemMake) product.product_make = this.decode(item.itemMake)
        ...
        if (item.itemIndustry) product.product_industry = this.decode(item.itemIndustry)
    }

    setProductCategorizationFromItem(item, product) {
        if (item.itemType) product.product_category = this.decode(item.itemType)
        if (item.itemTypeId) product.product_category_id = item.itemTypeId
        if (item.itemSubtype) product.product_subcategory = this.decode(item.itemSubtype)
...
class FormHandler {
    ...
    initialize(config) {
        ...
        // Add BZ_Form_Open event listener on Branded Zone Pages
        document.addEventListener('BZFormOpen', (e) => {
            var form = {}
            form.tealium_event = 'form_load'
            var productData = {}
            var item = e.detail
            var formDetail = item.FormDetailGuid 
    ...
    trackStaticFormLoads() {
        ...
        forms.forEach((form, index) => {
            ...
            if (form.closest('[class*="bz-"]')) {
                return // SKIP Branded Zone forms (forms on branded zone pages)
            }
            // SPARK-XXXX: The old template skipped all VDP-Unit-Detail lead forms here
            // because they were previously rendered as Bootstrap AriFormModals and tracked
            // via the `show.bs.modal` handler (see FormHandler.initialize). After the
            // storefront refactor, VDP lead forms (Request Info, Get More Info, Value Your
            // Trade, Insurance Quote, etc.) are rendered as inline .component LeadForm_*
            // blocks, so the blanket skip caused them to miss both form_load and
            // form_interaction entirely. Narrow the skip to forms that are still actually
            // inside an AriFormModal container — those are still covered by path (2) and
            // we don't want to double-fire form_load for them.
            if (!isCantFindContext && form.closest('div[id*="AriFormModal"]')) {
                return // Skip AriFormModal-wrapped forms (tracked via show.bs.modal handler)
            }
    ...
    // formInteraction: attach first-interaction tracking for the form
    // identified by `formDetail`. Uses **document-level event delegation**
    // so that tracking is race-proof against:
    //   (a) Alpaca inserting the <form> into a .ari-form wrapper AFTER
    //       Bootstrap's `show.bs.modal` fires (the "first-open silent,
    //       second-open fires" bug).
    //   (b) The entire modal / .ari-form container being inserted into
    //       the DOM lazily (e.g. AJAX-rendered modals or frameworks that
    //       mount nodes on click).
    //   (c) Alpaca tearing down and rebuilding the <form> element after
    //       initial render — listeners bound to a specific form node would
    //       be orphaned; listeners on `document` survive.
    //
    // We resolve the container at **event time** rather than call time via
    // getElementById(formDetail) + container.contains(target). If the
    // container doesn't exist yet when the user first interacts, we bail
    // silently and wait for the next event — as soon as the container
    // appears anywhere in the DOM, the very next input/focus/click on one
    // of its descendants fires tracking correctly.
    //
    // Overhead: three document-level listeners per formInteraction() call.
    // The handlers short-circuit on type/matches checks before doing any
    // real work and **remove themselves on first successful fire**, so the
    // sustained cost is bounded by "number of forms the user has not yet
    // interacted with on this page" — trivial in practice.
    formInteraction(final, formDetail, optionalParam = '') {
        if (!formDetail) return

        const formKey = this.getFormKey(final)

        // Delegated handler. Target filter mirrors the legacy jQuery
        // selector 'input,select,textarea,label' — so clicks on modal
        // chrome (close buttons, headers, unrelated labels) don't count.
        const handleFirstInteraction = (e) => {
            const t = e.target
            if (!t || typeof t.matches !== 'function') return
            if (!t.matches('input, select, textarea, label')) return

            // Resolve the container NOW (at event time). Handles the case
            // where the container was inserted into the DOM AFTER this
            // formInteraction() call returned.
            const container = document.getElementById(formDetail)
            if (!container || !container.contains(t)) return

            if (this.interactionTracked.has(formKey)) return
            this.interactionTracked.add(formKey)
            ...
            document.removeEventListener('input', handleFirstInteraction, true)
            document.removeEventListener('focus', handleFirstInteraction, true)
            document.removeEventListener('click', handleFirstInteraction, true)
        }

        // Capture phase (the `true` below) is required: `focus` and `blur`
        // don't bubble, and we want focusing a field to count. `input` and
        // `click` bubble normally, so capture is harmless for them.
        document.addEventListener('input', handleFirstInteraction, true)
