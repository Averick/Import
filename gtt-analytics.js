...
if (isBrandedZoneSite) {
	{{#each @root.CustomDataModel.pageMappings}}
		 // Check if absolute path matches any branded zone specific mapping 
		 // AND ensuring it matches the folder path to be safe contextually
...
// 2. Then check general page mappings, but ONLY if we haven't already identified it as a specific branded zone page
// Or if we want to overwrite standard logic. 
// However, the issue is generic logic overwriting specific logic.
// So we should reverse: Check generic first, then specific overwrite? 
// OR: prevent generic check if we are in a branded zone directory.

// Actually, the issue is that both blocks execute.
// Block 1 (Line 101): Matches branded zone folder path
// Block 2 (Line 109): Matches absolute path (generic)

// If we are on /shop-indian-motorcycle/search/inventory:
// Block 1 matches /shop-indian-motorcycle (via FolderPath config potentially) -> sets to "branded zone"
// Block 2 matches /search/inventory (via regex in CustomDataModel) -> overwrites to "search"

// We should structure this as: 
// Iterate all mappings. Find best match.
// Since we can't easily do "best match" in Handlebars linear execution, we should rely on order.
// Specific mappings should generally come LAST to overwrite generic ones, OR we guard the generic ones.

// Current Config:
// /shop-indian-motorcycle -> Matches "branded zone"
// /search/inventory -> Matches "search"

// The URL satisfies BOTH regexes if we look at partials, but valid mappings in CustomDataModel are:
// "^/shop-indian-motorcycle"
// "^\/search\/inventory"

// Wait, "^/search/inventory" starts with ^. 
// The URL is /shop-indian-motorcycle/search/inventory. 
// So "^/search/inventory" should NOT match.
// BUT... 
// "^\/search\/inventory\/(?!query)" -> this regex might be the culprit if strictly interpreted?
// No, ^ anchor means start of string.

// Let's look at the regexes again.
// Line 88: "^\/search\/inventory$"
// Line 80: "^\/search\/inventory\/(?=query)"

// If the regex IsMatch is checking the *absolute path*, then "^/search/inventory" would NOT match "/shop-indian-motorcycle/search/inventory".
// UNLESS the regex in json is missing the anchor or IsMatch helper behaves differently.
// Providing the fix by ensuring we don't run generic checks if we are in an OemProgram context and the URL starts with that program's folder path.

if (!isBrandedZoneSite || (isBrandedZoneSite && absolutePath.indexOf('{{@root.Site.Settings.OemProgram.FolderPath}}') !== 0)) {
	{{#each @root.CustomDataModel.pageMappings}}
